...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', default="./S04A04I01M0/detections/detection_result.pkl", help='path to the detection results', required=True)
        ...
        for i in range(len(bboxes)):
            if scores[i] > score_thresh:
                fid.write(frame_id + ',' + '-1' + ',' + str(bboxes[i][0]) + ',' + str(bboxes[i][1])
                          ...
                          + str(bboxes[i][3] - bboxes[i][1])+','+str(scores[i])+'\n')
