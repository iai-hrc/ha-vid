...
class Trainer:
    ...
    def train_bcn(self, save_dir, data_loader,  num_epochs, learning_rate, device, date, num,checkpoint_path, results_dir, features_path,actions_dict, sample_rate, dataset,
        ...
        # save test acc results for getting the best result
        test_acc_list = open(save_dir+'/test_acc_list.txt','w')
        ...
        for epoch in range(num_epochs):
            ...
            test_acc_list.write('{}, {}, {}, {}, {}\n'.format(test_acc, test_edit, test_f1[0],test_f1[1],test_f1[2]))
    ...
    def predict_bcn(self, model_dir, results_dir, features_path, vid_list_file, epoch, actions_dict,bgm_result_path, device, sample_rate, dataset, ground_truth_path,
        ...
        with torch.no_grad():
            ...
            evaluation_results = open(model_dir+'/evaluation_results.txt','w')
            acc, edit_result, f1_score = eval_metric(dataset,list_of_vids,ground_truth_path,results_dir+"/")
            evaluation_results.write(str(acc)+'\n')
            evaluation_results.write(str(edit_result)+'\n')
            evaluation_results.write(str(f1_score))
